...
export default function RequestDetailPage() {
  ...
  // Status configuration (aligned with MyRequestsPage)
  const statusConfig = {
    en_attente: {
      label: "En attente",
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const { data: admins, error: adminError } = await supabase
        .from("users")
        .select("id")
        .in("role", ["chef_dept", "directeur_prog", "dae", "secretaire_dg", "dg"]);
      ...
      if (admins) {
        const notifications = admins.map((admin) => ({
          user_id: admin.id,
          message: `Nouveau message de ${user.nom} pour la demande ${request.titre} (Ref: ${request.id})`,
          lu: false,
      ...
      toast({
        ...
        description: "Votre message a été transmis au service concerné.",
      });
    } catch (error: any) {
